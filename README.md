...
インストール後は `docker torii` として利用できる。
...
```
docker torii run -- nginx
docker torii run -n myproject -- -p 8080:80 nginx
docker torii run --env-path /app/.env -- myimage
```

torii で管理している環境変数を復号し、コンテナ内にファイルとしてマウントする。`docker run --env` と異なり、`docker inspect` やプロセスリストから平文が見えない。

- 復号した .env を tmpfs 上に書き出し（Linux）、bind mount でコンテナに渡す
- コンテナ終了後に .env を zeroize し tmpfs を削除
- Ctrl+C / SIGTERM 時もクリーンアップを保証
- コンテナ内のマウント先は `--env-path` で変更可能（デフォルト: `/secrets/.env`）
- tmpfs が利用できない環境では通常の一時ディレクトリにフォールバック
...
- Docker プラグインは tmpfs 上に .env を書き出し、コンテナ終了・シグナル中断時に zeroize して削除
