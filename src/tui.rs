...
pub fn run_interactive(db_path: &str) -> Result<()> {
    ...
    loop {
        ...
        let result = match choice {
            0 => interactive_set(db_path, &password),
            1 => interactive_get(db_path, &password),
            2 => cmd_list(db_path, &password),
            3 => interactive_delete(db_path, &password),
            4 => interactive_serve(db_path, &password),
            5 => break,
            _ => unreachable!(),
        };
...
fn interactive_set(db_path: &str, password: &str) -> Result<()> {
    ...
    let expires = if set_expires {
        let date: String = Input::new()
            .with_prompt("Expires (YYYY-MM-DD)")
            .interact_text()
            .map_err(io_err)?;
        Some(date)
    } else {
    ...
    cmd_set(db_path, password, &key_value, expires.as_deref())
}

fn interactive_get(db_path: &str, password: &str) -> Result<()> {
    let key: String = Input::new()
        .with_prompt("Key")
        .interact_text()
        .map_err(io_err)?;

    cmd_get(db_path, password, &key)
}

fn interactive_delete(db_path: &str, password: &str) -> Result<()> {
    let key: String = Input::new()
        .with_prompt("Key")
        .interact_text()
        .map_err(io_err)?;
    ...
    if confirmed {
        cmd_delete(db_path, password, &key)
    } else {
        eprintln!("Cancelled.");
        Ok(())
    }
...
fn interactive_serve(db_path: &str, password: &str) -> Result<()> {
